...
class Network(object):
            ...
            with tf.variable_scope(op_name, reuse=True):
                for param_name, data in iter(data_dict[op_name].items()):
                    try:
                        ...
                        var = tf.get_variable(param_name, initializer = data)
    ...
    def weight_var(self, name, shape):
        initial = tf.truncated_normal_initializer(mean= 0.0, stddev=0.005)
        return tf.get_variable(name, shape, dtype = 'float32', initializer = initial)

    def bias_var(self, name, shape):
    ...
    def conv(self,
        ...
        with tf.variable_scope(name) as scope:
            ...
            # Add the biases
            if biased:
                biases = self.bias_var('biases', [c_o])
    ...
    @layer
    def mul(self, inputs, name):
        list_inputs = tf.unstack(inputs, axis=2)
        return tf.multiply(list_inputs[0], list_inputs[1])

    @layer
    def elu(self, input_data, name):
        return tf.nn.elu(input_data, name=name)
    ...
    def batch_normalization(self, input_data, name, scale_offset=True, relu=False):
        ...
        #with tf.variable_scope(name) as scope:
            ...
            '''
            output = tf.contrib.layers.batch_norm(input_data, decay=0.999, updates_collections=None, epsilon=1e-4,
                                            center=True, scale=True, is_training=self.is_training, scope=name)
    ...
    @layer
    def dropout(self, input_data, name):
        return tf.nn.dropout(input_data, self.keep_prob, name=name)
